        ...
        <div className="h-screen flex! flex-col! w-68 border-r  border-gray-400">
            ...
            <div className="flex flex-col items-center justify-between flex-auto!">
                <div className="flex! flex-col">
                    ...
                    <Button onClick={() => setCurrentPage("games")} className={`${baseButtonStyle} ${currentPage == "games" ? activeButton :hoverButton}`}><LayoutDashboard />{t("games")}</Button>
                    <Button onClick={()=>setCurrentPage("calendar")} className={`${baseButtonStyle} ${currentPage == "calendar" ? activeButton :hoverButton}`}><Calendar />{t("calendar")}</Button>
                </div>
                <Button onClick={()=>setCurrentPage("settings")} className={`${baseButtonStyle} ${currentPage == "settings" ? activeButton :hoverButton}` }><Bolt />{t("settings")}</Button>
            </div>
        </div>)
}
